...
if [[ -n ${NAMESPACE} ]];then
  ns=$(minikube kubectl -- get namespace "${NAMESPACE}" --no-headers --output=go-template="{{.metadata.name}}" 2>/dev/null)
  if [[ -z ${ns} ]];then
...
for proto in "${protos[@]}"; do
  for resource in $(minikube kubectl -- get -n ${NAMESPACE} "$proto" -o name); do
    minikube kubectl -- delete -n ${NAMESPACE} "$resource";
  done
done

# clean up Gateway API resources that may have been used
if minikube kubectl -- get crd gateways.gateway.networking.k8s.io >/dev/null 2>&1; then
  ...
  minikube kubectl -- delete -n ${NAMESPACE} -f "$SCRIPTDIR/bookinfo-versions.yaml" >/dev/null 2>&1
fi

OUTPUT=$(mktemp)
export OUTPUT
echo "Application cleanup may take up to one minute"
minikube kubectl -- delete -n ${NAMESPACE} -f "$SCRIPTDIR/bookinfo.yaml" > "${OUTPUT}" 2>&1
ret=$?
function cleanup() {
  rm -f "${OUTPUT}"
}
